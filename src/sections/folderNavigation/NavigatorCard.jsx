...
function NavigatorCard(props) {
  const { updateCodeEditorText } = useContext(CodeEditorContext);
    ...
    getFoldersFilesFromParent(parentId)
      .then(async (foldersAndFiles) => {
        ...
        folders.forEach((folder) => {
          content.push({ ...folder, type: "folder" });
        });
        files.forEach((file) => {
          content.push({ ...file, type: "file" });
        });
  ...
  const handleDoubleClic = (child) => {
    if (child.type == "folder") {
      ...
      setPath({
        folderNames: [...path.folderNames, child.name],
    ...
    postDataContent(newFile, "file")
      .then((answer) => {
        path.folderIds.length < 1
          ? getContent("")
          : getContent(path.folderIds[path.folderIds.length - 1]);
      })
      .catch((error) => {
        console.log(error);
      });
      ...
      <div className="flex-none p-2 bg-secondary">
        <div className="flex relative">
          <div className="w-3/12 pt-1 bg-gray-600 rounded-l-lg text-center">
            <button className="text-white" onClick={() => backPath()}>
              Back
            </button>
          </div>
          <div className="w-8/12 p-1 border-2 border-gray-600 bg-white">
            MyDiagrams:/{path.folderNames.join("/")}
          </div>
        ...
        <div className="h-full p-2 flex flex-wrap">
          {childrenOfFolder.map((child) => (
            <div key={child._id}>
              <button
                onClick={() => {
                  setChildChoosen(child);
                }}
                onDoubleClick={() => {
                  handleDoubleClic(child);
                }}
              >
          ...
          <div className="w-1/4 border-2 p-1 border-gray-600 rounded-r-lg bg-tertiary text-black hover:bg-primary hover:text-white">
            {newFileName == "" ? (
              <button
                className="w-full"
                onClick={() => {
                  handleDoubleClic(childChoosen);
                }}
                disabled={childChoosen.name == ""}
              >
                Abrir
              </button>
            ) : (
